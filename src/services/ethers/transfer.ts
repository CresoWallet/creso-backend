...
export async function transferAllETH(wallet: ethers.Wallet, to: string): Promise<any> {
...
export async function transferETHFromSmartWallet(
    ...
    try {
        ...
        if (balance.lt(amountInWei.add(gasPrice.mul(gasLimit)))) {
            throw new Error("The smart wallet does not have enough balance to cover the transfer and gas costs.");
        }
